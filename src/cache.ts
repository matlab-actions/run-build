...
export const CACHE_PATH = ".buildtool";
...
export function getCacheKey(): { primaryKey: string; restoreKeys: string[] } {
    ...
    // The matrix values distinguish otherwise-identical legs of a matrix job
    // (e.g. different MATLAB releases). Hash them so the key stays short and
    // free of characters that are awkward in cache keys.
...
export async function saveCache(): Promise<void> {
    ...
    if (!primaryKey) {
        // Caching was disabled, so restoreCache never ran.
